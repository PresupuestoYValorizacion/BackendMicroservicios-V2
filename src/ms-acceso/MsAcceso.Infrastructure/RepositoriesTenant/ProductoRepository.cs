...
internal sealed class ProductoRepository : RepositoryTenant<Producto, ProductoId>, IProductoRepository
{
    ...
    {
        return await DbContext.Set<Producto>().Include(x => x.DetalleProducto).Include(x => x.Resenias).Include(x => x.Categorias).Include(x => x.ProductoCategorias).Where(x => x.Activo == new Activo(true)).ToListAsync(cancellationToken);
    }
